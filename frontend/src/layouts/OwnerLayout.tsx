...
export const OwnerLayout: React.FC = () => {
  ...
  const navItems = [
    ...
    { name: 'Trainers', path: '/owner/trainers', icon: <Users size={20} /> },
    { name: 'Membership Plans', path: '/owner/membership-plans', icon: <ClipboardList size={20} /> },
  ];

  // Safely get user info if needed
  const token = localStorage.getItem('gym_token');
  const decoded = token ? decodeToken(token) : null;
  const username = (decoded as any)?.unique_name || 'Owner';
    ...
    <div className="min-h-screen flex" style={{ backgroundColor: 'var(--color-bg-base)' }}>
      ...
      <aside className="w-64 bg-neutral-900 text-white flex flex-col shrink-0" style={{ backgroundColor: 'var(--color-neutral-900)' }}>
        <div className="h-16 flex items-center px-6 border-b border-neutral-800" style={{ borderColor: 'var(--color-neutral-800)' }}>
          <Dumbbell className="text-primary-500 mr-3" style={{ color: 'var(--color-primary-500)' }} />
          <span className="font-bold text-lg tracking-wide text-white">Gym Owner</span>
        </div>
